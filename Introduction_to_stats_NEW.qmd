...
identify_outliers(data = heart_disease, 
...
```{r}
chisq.test(table(heart_disease$BloodSugar,
...
```{r}
t.test(Cholesterol ~ BloodSugar, 
       data = heart_disease, var.equal = TRUE)
wilcox.test(Cholesterol ~ BloodSugar, 
            data = heart_disease)

ggboxplot(heart_disease, 
          x = "BloodSugar", 
          y = "Cholesterol", 
          fill = "BloodSugar")
```
...
```{r}
summary(aov(Cholesterol ~ ChestPain, 
            data = heart_disease))
kruskal.test(Cholesterol ~ ChestPain, 
             data = heart_disease)
...
```{r}
TukeyHSD(aov(Cholesterol ~ ChestPain, 
             data = heart_disease))
```
...
model1 <- glm(BloodSugar ~ HeartDisease, 
              data = heart_disease,
              family = "binomial")
summary(model1)
```
...
```{r}
# Linear regression
model2 <- lm(Cholesterol ~ HeartDisease,
             data = heart_disease)
summary(model2)
```
...
model_multi <- lm(Cholesterol ~ BP + MaximumHR,
                  data = heart_disease)
summary(model_multi)
