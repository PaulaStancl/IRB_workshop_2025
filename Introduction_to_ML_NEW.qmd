...
```{r}
# Select only the best features and standardize
...
coef_df <- as.data.table(coef(summary(glm_model$finalModel)),
...
# Evaluating the model
confusion_2 <- confusionMatrix(predictions_2,
                               testData$HeartDisease)
print(confusion_2)
```
...
test_probs <- predict(glm_model_back,
                      newdata = testData,
                      type = "prob")[, "Yes"]
